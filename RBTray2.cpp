...
void ExecuteMenu() {
    ...
    if (!hMenu) {
        MessageBox(NULL, L"Error creating menu.", L"RBTray2", MB_OK | MB_ICONERROR);
        return;
    }
    AppendMenu(hMenu, MF_STRING, IDM_ABOUT,   L"About RBTray2");
    AppendMenu(hMenu, MF_STRING, IDM_EXIT,    L"Exit RBTray2");
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE /*hPrevInstance*/, _In_ LPSTR /*szCmdLine*/, _In_ int /*iCmdShow*/) {
    ...
    //     } else {
    //         MessageBox(NULL, L"RBTray2 is already running.", L"RBTray2", MB_OK | MB_ICONINFORMATION);
    //     }
    //     return 0;
    // }

    if (!(_hLib = LoadLibrary(L"RBHook2.dll"))) {
        MessageBox(NULL, L"Error loading RBHook2.dll.", L"RBTray2", MB_OK | MB_ICONERROR);
        return 0;
    }
    if (!RegisterHook(_hLib)) {
        MessageBox(NULL, L"Error setting hook procedure.", L"RBTray2", MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    if (!RegisterClass(&wc)) {
        MessageBox(NULL, L"Error creating window class", L"RBTray2", MB_OK | MB_ICONERROR);
        return 0;
    }

    if (!(_hwndHook = CreateWindow(NAME, NAME, WS_OVERLAPPED, 0, 0, 0, 0, (HWND)NULL, (HMENU)NULL, (HINSTANCE)hInstance, (LPVOID)NULL))) {
        MessageBox(NULL, L"Error creating window", L"RBTray2", MB_OK | MB_ICONERROR);
        return 0;
    }
    ...
    // if (!registeredHotKey) {
    //     MessageBox(NULL, L"Couldn't register hotkey", L"RBTray2", MB_OK | MB_ICONERROR);
    // }
